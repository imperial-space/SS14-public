...
petting-success-derelict-cyborg = Вы тянетесь погладить {THE($target)} по {POSS-ADJ($target)} ржавая металлическая головка.
petting-success-station-ai-others = { CAPITALIZE(THE($user)) } Вы гладите {THE($target)} по {POSS-ADJ($target)} его холодным квадратным экране.
