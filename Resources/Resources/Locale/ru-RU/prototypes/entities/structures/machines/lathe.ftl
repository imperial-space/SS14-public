ent-Autolathe = автолат
    .desc = Он производит изделия из металла и стекла.
    .suffix = { "" }
ent-Protolathe = протолат
    .desc = Преобразует сырье в полезные предметы.
    .suffix = { "" }
ent-CircuitImprinter = принтер схем
    .desc = Печатает платы для машин.
    .suffix = { "" }
ent-PrinterDoc = принтер документов
    .desc = Бюрократическое совершенство. Хранит базу данных всех документов Nanotrasen и позволяет распечатывать их, пока у вас есть бумага.
    .suffix = { "" }
ent-ExosuitFabricator = фабрикатор экзокостюмов
    .desc = Печатает запчасти для создания робототехники и других механизмов.
    .suffix = { "" }
ent-SecurityTechFab = охранный ТехФаб
    .desc = Печатает снаряжение для службы безопасности и некоторые базовые вещи.
    .suffix = { "" }
ent-MedicalTechFab = медицинский ТехФаб
    .desc = Печатает снаряжение для медицинского отдела.
    .suffix = { "" }
ent-UniformPrinter = принтер униформы
    .desc = Печать новой или запасной униформы.
    .suffix = { "" }
ent-OreProcessor = переработчик руды
    .desc = Он производит металлические листы и слитки из руды.
    .suffix = { "" }
ent-Sheetifier = лист-мастер 2000
    .desc = Довольно мяссивное устройство.
ent-AmmoTechFab = ТехФаб аммуниции
    .desc = Печатает минимум пуль, который может понадобиться любой бюджетной армии или оружейной. Ничего особенного.
ent-CircuitImprinterHyperConvection = гиперконвекционный принтер плат
    .desc = Высокоэкспериментальный принтер плат, который использует силу экстремального тепла для медленного создания объектов более экономичным способом.
