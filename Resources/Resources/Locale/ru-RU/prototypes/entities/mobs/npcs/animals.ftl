ent-MobBat = летучая мышь
    .desc = В одних культурах они вызывают ужас, в других - хрустят на зубах.
    .suffix = { "" }
ent-MobBee = пчела
    .desc = Приятно иметь, но нельзя построить цивилизацию на фундаменте из одного только меда.
    .suffix = { "" }
ent-MobAngryBee = пчела
    .desc = Какая милая пчелка. О нет, она выглядит злой и хочет мою пиццу.
    .suffix = Злой
ent-MobChicken = курица
    .desc = Была раньше яйца, динозавром!
    .suffix = { "" }
ent-MobChicken1 = курица
    .desc = Была раньше яйца, динозавром!
    .suffix = { "" }
ent-MobChicken2 = курица
    .desc = Была раньше яйца, динозавром!
    .suffix = { "" }
ent-MobDuckMallard = кряква
    .desc = Очаровательная кряква, она пушистая и мягкая!
    .suffix = { "" }
ent-MobDuckWhite = белая утка
    .desc = Очаровательная белая уточка, она пушистая и мягкая!
    .suffix = { "" }
ent-MobDuckBrown = коричневая утка
    .desc = Очаровательная коричневая уточка, пушистая и мягкая!
    .suffix = { "" }
ent-MobButterfly = бабочка
    .desc = Вопреки заблуждениям, это не душа вашей бабушки.
    .suffix = { "" }
ent-MobCow = корова
    .desc = Муу.
    .suffix = { "" }
ent-MobCrab = краб
    .desc = В народе говорят, что его клешня вышибает дух из космонавтов за грубые высказывания. Будьте вежливы и терпимы для вашей же безопасности.
    .suffix = { "" }
ent-MobGoat = коза
    .desc = Её позвоночник состоит из длинных острых сегментов, неудивительно, что она такая ворчливая.
    .suffix = { "" }
ent-MobGoose = гусь
    .desc = Его желудок и разум - загадка, недоступная человеческому пониманию.
    .suffix = { "" }
ent-MobGorilla = горилла
    .desc = Крушит, ревёт, выглядит круто. Не стойте рядом с ней.
    .suffix = { "" }
ent-MobKangaroo = кенгуру
    .desc = Крупное сумчатое травоядное. У него мощные задние лапы с ногтями, напоминающими длинные когти.
    .suffix = { "" }
ent-MobKangarooBoxer = кенгуру-боксёр
    .desc = Крупное сумчатое травоядное. У него мощные задние лапы и... боксерские перчатки?
    .suffix = { "" }
ent-MobMonkey = обезьяна
    .desc = Новая церковь неодарвинистов действительно верит, что КАЖДОЕ животное произошло от обезьяны. На вкус они как свинина, а убивать их весело и приятно.
    .suffix = { "" }
ent-MobGuidebookMonkey = тренировочная обезьяна
    .desc = Специально обученная обезьяна, чья единственная цель в жизни - чтобы вы кликнули на неё. Можно ли считать, что обезьяна преподала вам урок?
    .suffix = { "" }
ent-MobMouse = мышь
    .desc = Пии!
    .suffix = { "" }
ent-MobMouse1 = { ent-MobMouse }
    .desc = { ent-MobMouse.desc }
    .suffix = { "" }
ent-MobMouse2 = { ent-MobMouse }
    .desc = { ent-MobMouse.desc }
    .suffix = { "" }
ent-MobMouseCancer = раковая мышь
    .desc = Токсично. Пии!
    .suffix = { "" }
ent-MobLizard = ящерица
    .desc = Я думаю, она потерялась.
    .suffix = { "" }
ent-MobSlug = слизняк
    .desc = И они называли это ящерицей?
    .suffix = { "" }
ent-MobFrog = лягушка
    .desc = Прыг прыг прыг. Выглядит мокрой.
    .suffix = { "" }
ent-MobParrot = попугай
    .desc = Проникает в ваши владения, шпионит за вами и при этом остается классным питомцем.
    .suffix = { "" }
ent-MobPenguin = пингвин
    .desc = Их жизнь - это постоянная боль из-за коленных суставов внутри тела.
    .suffix = { "" }
ent-MobGrenadePenguin = пингвин гренадёр
    .desc = Маленький пингвин с гранатой на шее. Заготавливаются Синдикатом на неблагоприятных ледяных планетах.
    .suffix = { "" }
ent-MobSnake = змея
    .desc = Хиссс! Укусы не ядовиты.
    .suffix = { "" }
ent-MobGiantSpider = тарантул
    .desc = Общепризнанно, что это буквально худшее существо на свете.
    .suffix = { "" }
ent-MobGiantSpiderAngry = тарантул
    .desc = { ent-MobGiantSpider.desc }
    .suffix = Злой
ent-MobGiantSpiderWizard = паук-волшебник
    .desc = Этот паук выглядит немного волшебно
    .suffix = Маг
ent-MobPossum = поссум
    .desc = "О поссум! Мой поссум!" -- Уолт Уитмен, 1865
    .suffix = { "" }
ent-MobRaccoon = енот
    .desc = Мусорная панда!
    .suffix = { "" }
ent-MobCorgi = корги
    .desc = Наконец-то, космический корги!
    .suffix = { "" }
ent-MobCorgiNarsi = порченный корги
    .desc = Иан! Нет!
    .suffix = { "" }
ent-MobCorgiPuppy = щенок корги
    .desc = Маленький корги! Оуу...
    .suffix = { "" }
ent-MobCat = кошка
    .desc = Питомец семейства кошачьих, очень забавный.
    .suffix = { "" }
ent-MobCatCalico = трёхцветная кошка
    .desc = Питомец семейства кошачьих, очень забавный.
    .suffix = { "" }
ent-MobCatSpace = космическая кошка
    .desc = Питомец семейства кошачьих, подготовленный к худшему.
    .suffix = { "" }
ent-MobCatCaracal = каракаловая кошка
    .desc = Весёлое создание природы.
    .suffix = { "" }
ent-MobSloth = ленивец
    .desc = Очень медлительное животное. Для людей с низкой энергией.
    .suffix = { "" }
ent-MobFerret = хорёк
    .desc = Просто маленький глупый парнишка!
    .suffix = { "" }
ent-MobFox = лиса
    .desc = Они - лисы.
    .suffix = { "" }
ent-MobHamster = хомяк
    .desc = Милый, пушистый, робастный хомяк.
    .suffix = { "" }
ent-MobPig = свинья
    .desc = Хрю.
    .suffix = { "" }
ent-MobBoxingKangaroo = боксёрский кенгуру
    .desc = Кенгуру с боксёрскими перчатками.
    .suffix = { "" }
ent-MobMouseAdmeme = { ent-MobMouse }
    .suffix = Адмемы
    .desc = { ent-MobMouse.desc }
ent-MobClownSpider = клоун-паук
    .desc = Сочетает в себе две самые страшные вещи на свете - пауков и клоунов.
ent-MobMonkeySyndicateAgent = { ent-MobMonkey }
    .desc = { ent-MobMonkey.desc }
ent-MobMouseDead = { ent-MobMouse }
    .suffix = Мёртвая
    .desc = { ent-MobMouse.desc }
ent-MobCockroach = таракан
    .desc = Эта станция просто кишит тараканами.
ent-MobGlockroach = тараган
    .desc = Эта станция просто кишит тар- О БОЖЕ, У ЭТОГО ТАРАКАНА ЕСТЬ ПИСТОЛЕТ!!!
    .suffix = Адмемы
ent-MobMothroach = таракамоль
    .desc = Это очаровательный побочный продукт многочисленных попыток генетического смешивания людей-мотыльков с тараканами.
ent-MobKobold = кобольд
    .desc = Кобольды, родственники Унатхов, прекрасно вписываются в свою естественную среду обитания и столь же противны, как обезьяны; готов вырвать тебе волосы и зарезать тебя до смерти.
ent-MobPossumOld = опоссум
    .desc = "О Опоссум! Мой Опоссум!" -- Уолт Уитмен, 1865
    .suffix = Старый
ent-MobCatShadow = теневой кот
    .desc = Прекрасный кусочек темноты. Надеюсь, он не навлечет на тебя проклятие.
ent-MobCatSyndy = Синдик
    .desc = Бомбезный котёнок
ent-MobDionaNymph = диона нимфа
    .desc = Это как кошка, только... Ветвистая.
ent-MobDionaNymphAccent = { ent-MobDionaNymph }
    .desc = { ent-MobDionaNymph.desc }
    .suffix = Акцент
ent-MobWatcherLavaland = наблюдатель
    .desc = Как будто он смотрит сквозь тебя.
ent-MobWatcherIcewing = ледокрылый наблюдатель
    .desc = { ent-MobWatcherLavaland.desc }
ent-MobWatcherMagmawing = магмакрылый наблюдатель
    .desc = { ent-MobWatcherLavaland.desc }
ent-MobWatcherPride = рардужный наблюдатель
    .desc = Этот редкий подвид появляется лишь в июне.
ent-MobMonkeySyndicateAgentNukeops = { ent-MobMonkey }
    .desc = { ent-MobMonkey.desc }
ent-MobBaseSyndicateMonkey = { ent-MobMonkey }
    .desc = { ent-MobMonkey.desc }
ent-MobBaseSyndicateKobold = { ent-MobKobold }
    .desc = { ent-MobKobold.desc }
    .suffix = Синдикат, База
ent-MobKoboldSyndicateAgent = { ent-MobKobold }
    .desc = { ent-MobKobold.desc }
    .suffix = Синдикат, Агент
ent-MobKoboldSyndicateAgentNukeops = { ent-MobKobold }
    .desc = { ent-MobKobold.desc }
    .suffix = Синдикат, Нюка
