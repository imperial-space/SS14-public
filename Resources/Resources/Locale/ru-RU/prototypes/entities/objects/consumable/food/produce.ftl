ent-ProduceBase = { ent-BaseItem }
    .desc = { ent-BaseItem.desc }
    .suffix = { "" }
ent-FoodProduceBase = { ent-FoodInjectableBase }
    .desc = { ent-FoodInjectableBase.desc }
    .suffix = { "" }
ent-WheatBushel = сноп пшеницы
    .desc = Вздох... пшеница... зерно?
    .suffix = { "" }
ent-OatBushel = сноп овса
    .desc = Ешьте овсянку, делайте зарядку.
    .suffix = { "" }
ent-Sugarcane = сахарный тростник
    .desc = Болезненно-сладкий.
    .suffix = { "" }
ent-Papercane = рулон бумажной тросточки
    .desc = Зачем нам вообще нужно выращивать бумагу?
    .suffix = { "" }
ent-Log = бревно
    .desc = Такое заметишь и в своём глазу.
    .suffix = { "" }
ent-Nettle = крапива
    .desc = Скупой маленький урод.
    .suffix = { "" }
ent-FoodBanana = банан
    .desc = Богат калием.
    .suffix = { "" }
ent-TrashBananaPeel = банановая кожура
    .desc = { ent-BaseItem.desc }
    .suffix = { "" }
ent-TrashBananaPeelExplosive = { ent-TrashBananaPeel }
    .suffix = Взрывчатка
    .desc = { ent-TrashBananaPeel.desc }
ent-FoodCarrot = морковь
    .desc = Полезна для глаз!
    .suffix = { "" }
ent-FoodCabbage = кочан капусты
    .desc = Фууууу. Капуста.
    .suffix = { "" }
ent-FoodGarlic = чеснок
    .desc = Вкусный, но с потенциально ужасным запахом.
    .suffix = { "" }
ent-FoodLemon = лимон
    .desc = Когда жизнь дает вам лимоны, будьте благодарны, что это не лаймы.
    .suffix = { "" }
ent-FoodLime = лайм
    .desc = Лечит космическую цингу, позволяет вести себя как космический пират.
    .suffix = { "" }
ent-FoodOrange = апельсин
    .desc = Полезный, оранжевый.
    .suffix = { "" }
ent-FoodPineapple = ананас
    .desc = Ммм, тропический.
    .suffix = { "" }
ent-FoodPotato = картофель
    .desc = Космические ирландцы умерли от голода после того, как погибли их картофельные посевы. К несчастью, они не могли ловить космических карпов, потому что они были во владениях королевы. Напомните об этом любому члену космической ИРА, и они сойдут с ума от злости.
    .suffix = { "" }
ent-FoodTomato = помидор
    .desc = Ты произносишь тоу-мат, а я произношу то-мат.
    .suffix = { "" }
ent-PuddleTomato = помидор
    .desc = Плюх.
    .suffix = { "" }
ent-FoodEggplant = баклажан
    .desc = Может, внутри сидит баклан?
    .suffix = { "" }
ent-FoodApple = яблоко
    .desc = Это маленький кусочек Эдема.
    .suffix = { "" }
ent-FoodGoldenApple = золотое яблоко
    .desc = Кажется кто-то громко кричит...
    .suffix = { "" }
ent-FoodGlasstle = стеклянница
    .desc = Хрупкое кристаллическое растение с множеством острых шипов.
    .suffix = { "" }
ent-FoodCorn = початок кукурузы
    .desc = Нужно немного масла! И немного приготовить...
    .suffix = { "" }
ent-FoodCornTrash = кочерыжка кукурузы
    .desc = Не осталось ни одного зернышка.
    .suffix = { "" }
ent-FoodOnion = лук
    .desc = Не о чем плакать.
    .suffix = { "" }
ent-FoodOnionRed = красный лук
    .desc = Фиолетовый, несмотря на название.
    .suffix = { "" }
ent-FoodMushroom = гроздь лисичек
    .desc = Cantharellus Cibarius: Эти веселые желтые маленькие грибы выглядят очень аппетитно!
    .suffix = { "" }
ent-ProduceSliceBase = { ent-FoodInjectableBase }
    .desc = { ent-FoodInjectableBase.desc }
    .suffix = { "" }
ent-FoodPineappleSlice = ломтик ананаса
    .desc = Ммм, тропический.
    .suffix = { "" }
ent-FoodOnionSlice = ломтик лука
    .desc = Не о чем плакать.
    .suffix = { "" }
ent-FoodOnionRedSlice = ломтик красного лука
    .desc = Фиолетовый, несмотря на название.
    .suffix = { "" }
ent-FoodChili = чили
    .desc = Острый, лучше не трогать глаза.
    .suffix = { "" }
ent-FoodAloe = алоэ
    .desc = Ароматное растение с успокаивающими свойствами.
    .suffix = { "" }
ent-FoodPoppy = мак
    .desc = Цветок, экстракты которого часто используются в производстве лекарств.
    .suffix = { "" }
ent-FoodLingzhi = линчжи
    .desc = Сильнодействующий лекарственный гриб. Не переборщите.
    .suffix = { "" }
ent-FoodAmbrosiaVulgaris = амброзия вульгарис
    .desc = Лекарственное растение. Может вызывать ощущение легкого веселья.
    .suffix = { "" }
ent-FoodGalaxythistle = галакточертополох
    .desc = Лекарственное растение, используемое для получения антитоксина.
    .suffix = { "" }
ent-FoodFlyAmanita = мухомор
    .desc = Аппетитно выглядящий гриб, как в мультфильмах.
    .suffix = { "" }
ent-FoodGatfruit = гатфрукт
    .desc = Вкусный, плод в форме пистолета, с толстой деревянистой плодоножкой.
    .suffix = { "" }
ent-RiceBushel = сноп риса
    .desc = Может быть перемолот в рис, идеально подходит для пудинга или саке.
    .suffix = { "" }
ent-FoodSoybeans = соевые бобы
    .desc = Для тех, кто не выносит вида старого доброго мяса.
    .suffix = { "" }
ent-FoodWatermelon = арбуз
    .desc = Круглый зеленый предмет, который можно порезать и съесть.
    .suffix = { "" }
ent-PuddleWatermelon = арбуз
    .desc = Шмяк.
    .suffix = { "" }
ent-FoodWatermelonSlice = ломтик арбуза
    .desc = Сочный зеленый и красный ломтик.
    .suffix = { "" }
ent-FoodGrape = виноград
    .desc = Пища императоров, жителей космической Франции (обычно в виде вина) и занятых мамочек. Когда-нибудь его можно будет использовать в производстве вина для бармена, если оно вдруг закончится.
    .suffix = { "" }
ent-TrashBananiumPeel = бананиумовая кожура
    .desc = Кожура от странного банана.
ent-FoodCocoaPod = стручок какао
    .desc = Шоколада много не бывает!
ent-TrashBananaPeelExplosiveUnarmed = банан
    .desc = Есть что-то необычное в этом банане.
ent-FoodBloodTomato = кровяной помидор
    .desc = Подождите-ка, это не кетчуп...
ent-FoodBungo = плод бунго
    .desc = Скромный плод бунго.
ent-FoodBungoPit = косточка бунго
    .desc = { ent-FoodInjectableBase.desc }
ent-FoodChilly = чилли
    .desc = Обжигающе холодный.
ent-FoodBlueTomato = синий помидор
    .desc = Он синий.
ent-FoodAmbrosiaDeus = амброзия деус
    .desc = Лекарственное растение, пользующееся огромным спросом. Может оказывать побочные эффекты.
ent-FoodMimana = миман
    .desc = Фаворит мима.
ent-TrashMimanaPeel = мимановая кожура
    .desc = { ent-TrashBananaPeel.desc }
ent-FoodKoibean = коибобы
    .desc = Эти бобы кажутся немного подозрыбтельными.
ent-DeathNettle = смертокрапива
    .desc = Смертокрапива жаждет крови.
ent-FoodBerries = ягоды
    .desc = На вид куча разных ягод.
ent-SteelLog = бревно сталешляпника
    .desc = Сталь не растёт на деревьях! Конечно, она растёт как грибы.
ent-TrashBakedBananaPeel = запечёная банановая кожура
    .desc = зачем...
ent-FoodDoughTortillaFlat = раскаттанное тесто для тортильи
    .desc = Расплющенный кусок теста для тортильи. Приготовьте его, чтобы получить оболочку тако.
ent-FoodLaughinPeaPod = смеющийся стручок гороха
    .desc = Любимое растение клоуна.
ent-FoodPeaPod = стручок гороха
    .desc = Любимое лакомство утки!
ent-FoodLily = лилия
    .desc = Красивый оранжевый цветок.
ent-FoodEggChickenFertilized = яйцо
    .desc = это яйцо!
    .suffix = Оплодотворённое, Курица
ent-FoodEggDuckFertilized = яйцо
    .desc = это яйцо!
    .suffix = Оплодотворённое, Утка
ent-FoodEggBoiled = сваренное яйцо
    .desc = Вкусное яйцо вкрутую
ent-FoodMeatSnake = мясо змеи
    .desc = Длинный кусок змеиного мяса, надеюсь, не ядовитый.
ent-FoodPumpkin = тыква
    .desc = Большая оранжевая... ягода. Серьезно.
ent-CarvedPumpkinSmall = резная тыква
    .desc = Традиционное жуткое украшение.
    .suffix = Маленькая
ent-CarvedPumpkin = резная тыква
    .desc = Традиционное жуткое украшение.
    .suffix = Средняя
ent-CarvedPumpkinLarge = резная тыква
    .desc = Традиционное жуткое украшение.
    .suffix = Большая
ent-PumpkinLanternSmall = светильник джека
    .desc = Резная тыква, излучающая жуткое свечение.
    .suffix = Маленькая
ent-PumpkinLantern = светильник джека
    .desc = Резная тыква, излучающая жуткое свечение.
    .suffix = Средняя
ent-PumpkinLanternLarge = светильник джека
    .desc = Резная тыква, излучающая жуткое свечение..
    .suffix = Большая
ent-FoodSpacemansTrumpet = труба космонавта
    .desc = Яркий цветок со слабым запахом свежескошенной травы. Прикосновение к цветку через некоторое время после контакта приводит к окрашиванию кожи, однако на большинство других поверхностей это явление не влияет.
ent-FoodChevre = сыр Шевре
    .desc = Мягкое полено сливочного Шевра.
ent-FoodChevreSlice = кусочек Шевре
    .desc = Маленький диск сливочного Шевра. Идеальное украшение для французских гарниров.
ent-CottonBol = хлопок
    .desc = Любимая закуска молей, такая же пушистая, как и они.
ent-FoodLemoon = Лемун
    .desc = Люди говорят, что Луна сделана из сыра, но на самом деле Луна сделана из молока и лавра!
ent-FoodChiliPepper = перец чили
    .desc = Остро, лучше не трогать глаза.
ent-FoodChillyPepper = острый перец чили
    .desc = Ледяной жар.
ent-PyrottonBol = пироттоновый шарик
    .desc = Это, вероятно, заставит вас загореться.
