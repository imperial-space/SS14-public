...
public sealed class AtmosphericRCDSystem : EntitySystem
{
    ...
    private readonly ProtoId<AtmosphericRCDPrototype> _deconstructTileProto = "DeconstructTile";
    private readonly ProtoId<AtmosphericRCDPrototype> _deconstructLatticeProto = "DeconstructLattice";
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<AtmosphericRCDComponent, MapInitEvent>(OnMapInit);
        SubscribeLocalEvent<AtmosphericRCDComponent, ExaminedEvent>(OnExamine);
        SubscribeLocalEvent<AtmosphericRCDComponent, AfterInteractEvent>(OnAfterInteract);
        SubscribeLocalEvent<AtmosphericRCDComponent, RCDDoAfterEvent>(OnDoAfter);
        SubscribeLocalEvent<AtmosphericRCDComponent, DoAfterAttemptEvent<RCDDoAfterEvent>>(OnDoAfterAttempt);
        SubscribeLocalEvent<AtmosphericRCDComponent, AtmosphericRCDSystemMessage>(OnAtmosphericRCDSystemMessage);
        SubscribeNetworkEvent<AtmosphericRCDConstructionGhostRotationEvent>(OnAtmosphericRCDconstructionGhostRotationEvent);
    }

    #region Event handling

    private void OnMapInit(EntityUid uid, AtmosphericRCDComponent component, MapInitEvent args)
    {
    ...
    private void OnAtmosphericRCDSystemMessage(EntityUid uid, AtmosphericRCDComponent component, AtmosphericRCDSystemMessage args)
    {
    ...
    private void OnExamine(EntityUid uid, AtmosphericRCDComponent component, ExaminedEvent args)
    {
        ...
        if (prototype.Mode == AtmosphericRcdMode.ConstructTile || prototype.Mode == AtmosphericRcdMode.ConstructObject)
        {
    ...
    private void OnAfterInteract(EntityUid uid, AtmosphericRCDComponent component, AfterInteractEvent args)
    {
        ...
        {
            case AtmosphericRcdMode.Deconstruct:

                // Deconstructing an object
                if (args.Target != null)
                {
                    if (TryComp<AtmosphericRCDDeconstructableComponent>(args.Target, out var destructible))
                    {
            ...
            case AtmosphericRcdMode.ConstructTile:
    ...
    private void OnDoAfterAttempt(EntityUid uid, AtmosphericRCDComponent component, DoAfterAttemptEvent<RCDDoAfterEvent> args)
    {
    ...
    private void OnDoAfter(EntityUid uid, AtmosphericRCDComponent component, RCDDoAfterEvent args)
    {
        if (args.Cancelled)
        {
    ...
    private void OnAtmosphericRCDconstructionGhostRotationEvent(AtmosphericRCDConstructionGhostRotationEvent ev, EntitySessionEventArgs session)
    {
        ...
        if (!TryComp<AtmosphericRCDComponent>(uid, out var rcd))
            return;
    ...
    public bool IsRCDOperationStillValid(EntityUid uid, AtmosphericRCDComponent component, EntityUid gridUid, MapGridComponent mapGrid, TileRef tile, Vector2i position, EntityUid? target, EntityUid user, bool popMsgs = true)
    {
        var prototype = _protoManager.Index(component.ProtoId);
        ...
        {
            if (popMsgs)
                _popup.PopupClient(Loc.GetString("atmospheric-rcd-component-no-ammo-message"), uid, user);
        ...
        {
            if (popMsgs)
                _popup.PopupClient(Loc.GetString("atmospheric-rcd-component-insufficient-ammo-message"), uid, user);
        ...
        {
            case AtmosphericRcdMode.ConstructTile:
            case AtmosphericRcdMode.ConstructObject:
    ...
    private bool IsConstructionLocationValid(EntityUid uid, AtmosphericRCDComponent component, EntityUid gridUid, MapGridComponent mapGrid, TileRef tile, Vector2i position, EntityUid user, bool popMsgs = true)
    {
        var prototype = _protoManager.Index(component.ProtoId);

        // Check rule: Must build on empty tile
        if (prototype.ConstructionRules.Contains(AtmosphericRcdConstructionRule.MustBuildOnEmptyTile) && !tile.Tile.IsEmpty)
        {
            if (popMsgs)
        ...
        if (!prototype.ConstructionRules.Contains(AtmosphericRcdConstructionRule.CanBuildOnEmptyTile) && tile.Tile.IsEmpty)
        {
            if (popMsgs)
        ...
        if (prototype.ConstructionRules.Contains(AtmosphericRcdConstructionRule.MustBuildOnSubfloor) && !tile.Tile.GetContentTileDefinition().IsSubFloor)
        {
            if (popMsgs)
        ...
        // Tile specific rules
        if (prototype.Mode == AtmosphericRcdMode.ConstructTile)
        {
        ...
        var isWindow = prototype.ConstructionRules.Contains(AtmosphericRcdConstructionRule.IsWindow);
        var isCatwalk = prototype.ConstructionRules.Contains(AtmosphericRcdConstructionRule.IsCatwalk);
    ...
    private bool IsDeconstructionStillValid(EntityUid uid, TileRef tile, EntityUid? target, EntityUid user, bool popMsgs = true)
    {
        ...
        else
        {
            ...
            if (!TryComp<AtmosphericRCDDeconstructableComponent>(target, out var deconstructible) || !deconstructible.Deconstructable)
            {
                if (popMsgs)
    ...
    private void FinalizeRCDOperation(EntityUid uid, AtmosphericRCDComponent component, EntityUid gridUid, MapGridComponent mapGrid, TileRef tile, Vector2i position, Direction direction, EntityUid? target, EntityUid user)
    {
        if (!_net.IsServer)
            return;
        ...
        switch (prototype.Mode)
        {
            ...
            case AtmosphericRcdMode.ConstructObject:
                ...
                switch (prototype.AtmosphericRotation)
                {
                    case AtmosphericRcdRotation.Fixed:
                        Transform(ent).LocalRotation = Angle.Zero;
                        break;
                    case AtmosphericRcdRotation.Camera:
                        Transform(ent).LocalRotation = Transform(uid).LocalRotation;
                        break;
                    case AtmosphericRcdRotation.User:
