...
public sealed class InjectSystem : EntitySystem
{
    ...
    private void UpdateThing(EntityUid uid, InjectComponent component, UpdateEvent args)
    {
        ...
        if (!_solutions.TryGetSolution((ui, Comp<SolutionContainerManagerComponent>(ui)), "beaker", out var solu, out var solution))
            return;

        var removedSolution = _solutions.SplitSolution((solu.Value.Owner, solu.Value.Comp), args.Realtransfer.Value);
    ...
    private void Inject(EntityUid uid, InjectComponent component, InjectionEvent args)
    {
        ...
        if (_actionsSystem.TryGetActionData(args.Action, out var action))
        {
