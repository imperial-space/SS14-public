...
/// <summary>
/// Запускает личное для жертвы окно отравления едой.
/// Первые подходящие блюда, которых она коснётся, получают настоящий сохраняющийся яд.
/// </summary>
public sealed class DeathNoteGuidedFoodPoisoningPresetHandlerSystem : DeathNotePresetHandlerSystem,
    ...
    IDeathNotePresetPreludeHandler
{
    ...
    public DeathNotePresetHandlerType HandlerType => DeathNotePresetHandlerType.GuidedFoodPoisoning;
    ...
    public DeathNotePresetExecutionResult BeginPrelude(
        ...
        => _guidance.Begin(context, parameters, DeathNoteGuidedScenarioType.PoisonedFood);

    public DeathNotePresetExecutionResult Execute(
        in DeathNotePresetExecutionContext context,
        DeathNotePresetParameters parameters)
        => _guidance.Finish(context, DeathNoteGuidedScenarioType.PoisonedFood);
}
