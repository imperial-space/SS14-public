...
public sealed class ImperialScatteringGrenadeSystem : SharedImperialScatteringGrenadeSystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<ImperialScatteringGrenadeComponent, TriggerEvent>(OnScatteringTrigger);
    }
    ...
    /// </summary>
    private void OnScatteringTrigger(Entity<ImperialScatteringGrenadeComponent> entity, ref TriggerEvent args)
    {
    ...
    public override void Update(float frametime)
    {
        ...
        var query = EntityQueryEnumerator<ImperialScatteringGrenadeComponent>();

        while (query.MoveNext(out var uid, out var component))
        {
            var totalCount = component.FillPrototype.Count;
    ...
    /// </summary>
    private bool TrySpawnContents(MapCoordinates spawnCoordinates, ImperialScatteringGrenadeComponent component, out EntityUid contentUid)
    {
        contentUid = default;

        if (component.FillPrototype.Count > 0)
        {
            contentUid = Spawn(component.FillPrototype.First(), spawnCoordinates);
            component.FillPrototype.Remove(component.FillPrototype.First());
            return true;
        }
